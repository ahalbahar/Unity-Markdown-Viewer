...
5. **Test your changes** thoroughly:
   ...
   - Test Mermaid and PlantUML fenced blocks in formatted and raw view, including wide-chart scrolling, the `Expand` preview action, zoom/pan in the preview window, the fallback text path when diagram image fetches fail, and the backup render endpoint path
   ...
   - Test diagram disk caching from `Preferences > AB > Markdown`, including both `Library` and `Temp` cache locations when changing related code
...
- **PlantUML**: Test fenced `plantuml`, `puml`, and `uml` blocks, verify diagrams render in formatted view, verify shorthand sequence diagrams without `@startuml` tags, verify specialized starts such as `@startmindmap`, and verify the fallback text path when the PlantUML server and backup endpoint are unavailable
- **Diagram cache**: Verify the preference toggle and storage location selector under `Preferences > AB > Markdown`, and confirm cached diagrams refresh when expected
